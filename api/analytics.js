...
function transformResponse(batchResponse) {
  ...
  const daily = (reports[1]?.rows || []).map(function(row) {
    return {
  ...
  const pages = (reports[2]?.rows || []).map(function(row) {
    return {
  ...
  const sources = (reports[3]?.rows || []).map(function(row) {
    return {
  ...
  const countries = (reports[4]?.rows || []).map(function(row) {
    return {
...
module.exports = async function handler(req, res) {
  ...
  // Handle various env var formats for the private key
